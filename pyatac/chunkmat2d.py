...
class ChunkMat2D:
    ...
    def get(self, lower=None, upper=None, start=None, end=None, flip=False):
        ...
        if not flip:
            try:
                ...
                raise Exception(
                    "Looks like dimensions from get probaby don't match Mat"
                )
        else:
    ...
    def save(self, filename):
        ...
        head = ",".join(
            map(str, [self.chrom, self.start, self.end, self.lower, self.upper])
        )
    ...
    def getIns(self):
        ...
        insertion_track = InsertionTrack(
            self.chrom,
    ...
    def plot(self, filename=None, title=None, lower=None, upper=None):
        """Plot 2d ReadMat"""
        if upper is None:
            upper = self.upper
        if lower is None:
        ...
        fig = plt.figure()
        plt.imshow(
            self.get(lower=lower, upper=upper),
            origin="lower",
            interpolation="nearest",
            extent=[self.start, self.end - 1, lower, upper - 1],
...
class FragmentMat2D(ChunkMat2D):
    ...
    def makeFragmentMat(self, bamfile):
        """Make 2D matrix"""
        self.mat = makeFragmentMat(
            bamfile, self.chrom, self.start, self.end, self.lower, self.upper, self.atac
        )
...
class BiasMat2D(ChunkMat2D):
    ...
    def normByInsertDist(self, insertsizes):
        ...
        self.mat = self.mat * np.reshape(
            np.tile(inserts, self.mat.shape[1]), self.mat.shape, order="F"
        )
