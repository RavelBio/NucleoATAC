...
class Chunk:
    ...
    def asBed(self):
        ...
        out = "\t".join(
            map(
                str,
                [self.chrom, self.start, self.end, self.weight, self.name, self.strand],
            )
        )
        return out
...
class ChunkList(list):
    ...
    def isSorted(self):
        ...
        return all(
            [_chunkCompare(self[i], self[i + 1]) == -1 for i in range(len(self) - 1)]
        )
    ...
    def merge(self, new=False, sep=-1):
        ...
        for i in range(1, len(self)):
            if self[i].chrom == previous.chrom and self[i].start <= (
                previous.end + sep
            ):
    ...
    def read(
        ...
        for line in infile:
            ...
            if end - start >= min_length:
                out.append(
                    Chunk(
                        in_line[0], start, end, weight=weight, strand=strand, name=name
                    )
                )
        infile.close()
        if chromDict is not None and len(bad_chroms) > 0:
            ...
            warn_message = (
                str(len(bad_chroms))
                + " chromosome names in bed file not included in "
                + chrom_source
                + ":\n"
                + "\n".join(bad_chroms)
                + "\n "
    ...
    def convertChromSizes(chromDict, splitsize=None, offset=0):
        ...
        if splitsize is None:
            return ChunkList(
                *(Chunk(chrom, offset, chromDict[chrom] - offset) for chrom in chrs)
            )
        else:
            out = ChunkList()
            for chrom in chrs:
                out.extend(
                    ChunkList(
                        *(
                            Chunk(
                                chrom, i, min(i + splitsize, chromDict[chrom] - offset)
                            )
    ...
    def checkChroms(
        self,
        chroms,
        chunklist_source="bed file",
        chrom_source="fasta file",
        warn="Regions on these chromosomes will be ignored in analysis",
    ):
