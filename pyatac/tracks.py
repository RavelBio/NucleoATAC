...
class Track(Chunk):
    ...
    def assign_track(self, vals, start=None, end=None):
        ...
        if len(vals) != self.end - self.start:
            raise Exception(
                "The values being assigned to track do not \
    ...
    def write_track(self, handle, start=None, end=None, vals=None, write_zero=True):
        ...
        if len(vals) != self.end - self.start:
            ...
            raise Exception(
                "Error! Inconsistency between length of \
        ...
        for i in range(len(vals)):
            if vals[i] == prev_value:
                ...
                if write_zero or prev_value != 0:
                    output += (
                        "\t".join(
                            map(str, [self.chrom, start_range, start + i, prev_value])
                        )
                        + "\n"
                    )
                start_range = start + i
        ...
        if prev_value == 0:
            if write_zero:
                output += (
                    "\t".join(map(str, [self.chrom, start_range, end, prev_value]))
                    + "\n"
                )
    ...
    def smooth_track(self, window_len, window="flat", sd=None, mode="valid", norm=True):
        ...
        self.smoothed = True
        self.vals = smooth(
            self.vals, window_len, window=window, sd=sd, mode=mode, norm=norm
        )
        if mode == "valid":
    ...
    def get(self, start=None, end=None, pos=None):
        ...
        if pos:
            ...
            try:
                ...
                raise Exception(
                    "Looks like dimensions from get probaby don't match track, or there are no vals in track"
                )
...
class InsertionTrack(Track):
    ...
    def calculateInsertions(self, bamfile, flank=0, lower=0, upper=2000, atac=True):
        ...
        self.start = self.start - flank
        self.end = self.end + flank
        self.vals = getInsertions(
            bamfile, self.chrom, self.start, self.end, lower, upper, atac
        )

    def calculateStrandedInsertions(
        self, bamfile, flank=0, lower=0, upper=2000, atac=True
    ):
        """Compute inserion track for plus and minus strands separately"""
        self.start = self.start - flank
        self.end = self.end + flank
        self.plus, self.minus = getStrandedInsertions(
            bamfile, self.chrom, self.start, self.end, lower, upper, atac
        )
        self.vals = self.plus + self.minus

    def getInsertionSequences(
        self, fasta, nucleotides=["C", "G", "A", "T"], up=10, down=10
    ):
        ...
        for i in range(self.length()):
            mat += (
                self.vals[i] * seq_mat[:, (offset + i - up) : (offset + i + down + 1)]
            )
        return mat
    ...
    def getStrandedInsertionSequences(
        ...
        for i in range(self.length()):
            mat += (
                self.plus[i] * seq_mat[:, (offset + i - up) : (offset + i + down + 1)]
            )
            mat += self.minus[i] * np.fliplr(
                minus_seq_mat[:, (offset + i - down) : (offset + i + up + 1)]
            )
        return mat
...
class CoverageTrack(Track):
    ...
    def calculateCoverage(self, mat, lower, upper, window_len):
        ...
        if offset < 0:
            raise Exception(
                "Insufficient flanking region on mat to calculate coverage with desired window"
            )
        ...
        self.vals = smooth(
            collapsed, window_len, window="flat", mode="valid", norm=False
        )
    ...
    def calculateCoverageSmooth(self, mat, lower, upper, window_len, sd):
        ...
        if offset != 0:
            ...
            collapsed = np.sum(mat.mat[lower:upper,], axis=0)
        self.vals = smooth(
            collapsed, window_len, sd=sd, window="gaussian", mode="valid", norm=False
        )
