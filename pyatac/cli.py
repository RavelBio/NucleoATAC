...
def pyatac_main(args):
    ...
    if call == "vplot":
        ...
        if args.bg is not None and args.fasta is not None:
            print(
                "Warning-- both --bg and --fasta provided-- will us --bg for bias track and inrore --fasta"
            )
        print(
            "---------Making Bias VPlot------------------------------------------------------"
        )
        ...
        print(
            "---------Getting insertions to make track---------------------------------------"
        )
        get_ins(args)
    elif call == "cov":
...
def pyatac_parser():
    ...
    argparser = argparse.ArgumentParser(
        description="%(prog)s -- Utilitis for working with ATAC-Seq data",
...
def add_counts_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "counts", help="pyatac function-- compute fragment counts within windows"
    )
    group0 = parser.add_argument_group("Required", "Necessary arguments")
    group0.add_argument(
        "--bam", metavar="bam_file", help="Aligned reads", required=True
    )
    group0.add_argument(
        "--bed",
        metavar="bed_file",
    ...
    group3.add_argument(
        ...
        dest="atac",
        default=True,
        help="Don't use atac offsets",
    )
    group4 = parser.add_argument_group(
        "Fragment size bounds option", "Upper and lower limits"
    )
    group4.add_argument(
        "--lower",
        metavar="int",
        help="lower limit on insert \
        size. Default is 0",
        default=0,
        type=int,
    )
...
def add_sizes_parser(subparsers):
    ...
    """
    parser = subparsers.add_parser(
        "sizes", help="pyatac function-- compute fragment size distribution"
    )
    group0 = parser.add_argument_group("Required", "Necessary arguments")
    group0.add_argument(
        "--bam", metavar="bam_file", help="Aligned reads", required=True
    ...
    group4 = parser.add_argument_group(
        "Fragment sizs bounds option", "Upper and lower limits"
    )
    group4.add_argument(
        "--lower",
        metavar="int",
    ...
    group5.add_argument(
        "--no_plot", action="store_true", default=False, help="Don't plot output"
    )
    return
...
def add_bias_parser(subparsers):
    ...
    """
    parser = subparsers.add_parser(
        "bias", help="pyatac function-- compute Tn5 bias score"
    )
    group0 = parser.add_argument_group("Required", "Necessary arguments")
    group0.add_argument(
        "--fasta", metavar="fasta_file", help="Accepts fasta file", required=True
    )
    group1 = parser.add_argument_group(
        "PWM option", "Designate PWM file or default will be used"
    )
    group1.add_argument(
        "--pwm",
        metavar="Tn5_PWM",
        help="PWM descriptor file. Default is Human.PWM.txt included in package",
        default="Human",
    )
    ...
    group2.add_argument(
        "--bed",
        metavar="bed_file",
        help="Positions around which \
        to get nucleotide frequencies",
    )
    group3 = parser.add_argument_group("Options", "Optional settings")
    group3.add_argument("--out", metavar="output_basename", help="Basename for output")
    group3.add_argument(
        "--cores", metavar="int", help="number of cores to use", default=1, type=int
    )
    return
...
def add_pwm_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "pwm",
        help="pyatac function-- get nucleotide (or di-nucleotide) content around sites",
    )
    ...
    group1.add_argument(
        "--bam",
        metavar="bam_file",
        help="Reads around which to get nucleotide freq",
        required=True,
    )
    group2 = parser.add_argument_group(
        "Input sites",
        "If bed provided, find nucleotide frequencies around read starts within bed region",
    )
    group2.add_argument(
        "--bed", metavar="bed_file", help="Regions from which to use reads"
    )
    group3 = parser.add_argument_group("Options", "Optional settings")
    group3.add_argument(
        "--dinucleotide",
        action="store_true",
        help="Compute dinucleotide frequencies instead of single nucleotide",
        default=False,
    )
    group3.add_argument(
        "--flank",
        metavar="int",
        help="Bases away from insertion site to get frequencies for. Default is 10",
        default=10,
        type=int,
    )
    group3.add_argument(
        "--lower",
        metavar="int",
        help="lower limit on insert size. default is 0",
        default=0,
        type=int,
    )
    group3.add_argument(
        "--upper",
        metavar="int",
        help="upper limit on insert size. default is 2000",
        default=2000,
        type=int,
    )
    group3.add_argument(
...
def add_nucleotide_parser(subparsers):
    ...
    group3.add_argument(
        "--up",
        metavar="int",
        help="Bases upstream of site to get frequencies for",
        default=250,
        type=int,
    )
    group3.add_argument(
        "--down",
        metavar="int",
        help="Bases downstream of site to get frequencies for",
        default=250,
        type=int,
    )
    group3.add_argument(
        "--strand",
        metavar="int",
        help="Column in bedfile with strand info (1-based)",
        type=int,
    )
    group3.add_argument("--out", metavar="output_basename", help="Basename for output")
    group3.add_argument(
    ...
    group3.add_argument(
        ...
        action="store_true",
        default=False,
        help="Normalize by background frequencies",
    )
    return
...
def add_vplot_parser(subparsers):
    ...
    group1.add_argument(
        "--bam", metavar="bam_file", help="Accepts sorted BAM file", required=True
    )

    group2 = parser.add_argument_group("General options", "")
    group2.add_argument("--out", metavar="basename")
    group2.add_argument(
        "--cores", metavar="int", default=1, help="Number of cores to use", type=int
    )
    ...
    group3.add_argument(
        "--scale", action="store_true", default=False, help="Scale each site"
    )
    group3.add_argument(
        "--weight",
        metavar="int",
        type=int,
        help="column in which weight information is included",
    )
    group3.add_argument(
        "--strand",
        metavar="int",
        type=int,
        help="column in which strand information is included",
    )
    group3.add_argument(
        "--not_atac",
        action="store_false",
    ...
    group5.add_argument(
        "--plot_extra", action="store_true", default=False, help="Make some extra plots"
    )
    return
...
def add_bias_vplot_parser(subparsers):
    ...
    group0.add_argument(
        "--sizes",
        metavar="sizes_file",
        help="Accepts sizes file from pyatac sizes command",
        required=True,
    )
    group1 = parser.add_argument_group(
        "Bias Options",
    ...
    group1.add_argument(
        "--fasta", metavar="fasta_file", help="Accepts indexed fasta file"
    )
    group1.add_argument(
        "--pwm",
        metavar="Tn5_PWM",
...
def add_signal_parser(subparsers):
    ...
    """
    parser = subparsers.add_parser(
        "signal", help="pyatac function-- get signal around sites"
    )
    group1 = parser.add_argument_group("Required", "Necessary arguments")
    group1.add_argument(
        "--bed",
    ...
    group1.add_argument(
        "--bg",
        metavar="bg_file",
        help="Accepts bedgraph file that is tabix indexed",
        required=True,
    )
    group1.add_argument(
        "--sizes",
        metavar="genome_sizes_file",
        required=True,
        help="File with chromosome names in 1st col, sizes in 2nd",
    )
    group2 = parser.add_argument_group("General options", "")
    group2.add_argument("--out", metavar="basename", help="basename for output")
    group2.add_argument(
        "--cores", metavar="int", type=int, default=1, help="Number of cores to use"
    )
    group2.add_argument(
        "--all",
        action="store_true",
    ...
    group3 = parser.add_argument_group(
        "Bed options", "Options related to bed intervals"
    )
    group3.add_argument(
        "--up",
        metavar="int",
        help="bases upstream of site to look",
        type=int,
        default=250,
    )
    group3.add_argument(
        "--down",
        metavar="int",
        help="bases dowstream site to look",
        type=int,
        default=250,
    )
    group3.add_argument(
    ...
    group4.add_argument(
        "--exp", action="store_true", default=False, help="take exponent of value"
    )
    group4.add_argument(
        "--positive",
        action="store_true",
...
def add_ins_parser(subparsers):
    ...
    group2 = parser.add_argument_group("General options", "")
    group2.add_argument(
        "--bed", metavar="bed_file", help="Regions in which to get insertions"
    )
    group2.add_argument("--out", metavar="basename")
    group2.add_argument(
        "--cores", metavar="int", default=1, help="Number of cores to use", type=int
