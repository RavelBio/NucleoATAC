...
class VMat:
    ...
    def symmetrize(self):
        ...
        for j in range(self.lower, self.upper):
            ...
            if j % 2 == 1:
                lefthalf = (
                    self.mat[i, : (self.w + 1)] + self.mat[i, self.w :][::-1]
                ) * 0.5
                ...
                self.mat[i, :] = np.hstack(
                    (righthalf[::-1], righthalf, self.mat[i, -1])
                )
    ...
    def smooth(self, sd=1):
        ...
        self.mat = ndimage.filters.gaussian_filter(self.mat, sd, mode="constant")

    def smooth1d(self, sd=1, axis=1):
        """smooth v-plot along one axis only"""
        self.mat = ndimage.filters.gaussian_filter1d(self.mat, sd, axis, mode="nearest")
    ...
    def norm_y(self, dist):
        ...
        for i in range(self.mat.shape[0]):
            self.mat[i] = self.mat[i] * (
                dist.get(size=i + self.lower) / np.sum(self.mat[i])
            )
    ...
    def plot(self, mat=None, title=None, filename=None):
        ...
        if mat is None:
            ...
            raise VMat_Error(
                "dimensions of input mat should match \
                                dim of vmat"
            )
        fig = plt.figure()
        plt.imshow(
    ...
    def open(filename):
        ...
        try:
            ...
            raise VMat_Error(
                "VMat decriptor file appeas to be missing some\
needed components"
            )
        infile.close()
        return new
