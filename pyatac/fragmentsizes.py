...
class FragmentSizes:
    ...
    def calculateSizes(self, bamfile, chunks=None):
        if chunks is None:
            ...
            sizes = getFragmentSizesFromChunkList(
                chunks, bamfile, self.lower, self.upper, atac=self.atac
            )
    ...
    def get(self, lower=None, upper=None, size=None):
        if size:
            ...
            try:
                ...
                raise Exception(
                    "Looks like dimensions from get probaby don't match FragmentSizes"
                )
    ...
    def open(filename):
        ...
        for line in infile:
            if "#lower" in line:
                ...
                fragmentsizes = np.array(
                    list(map(float, line.rstrip("\n").split("\t")))
                )
        try:
            new = FragmentSizes(lower, upper, vals=fragmentsizes)
        except NameError:
            raise Exception(
                "FragmentDistribution decriptor file appeas to be missing some\
needed components"
            )
        infile.close()
        return new
