...
def run_nfr(args):
    ...
    if args.fasta is not None:
        ...
        chunks = ChunkList.read(
            args.bed, chromDict=chrs_fasta, min_offset=max(pwm.up, pwm.down)
        )
    else:
    ...
    pysam.tabix_compress(
        args.out + ".nfrpos.bed", args.out + ".nfrpos.bed.gz", force=True
    )
    shell_command("rm " + args.out + ".nfrpos.bed")
    pysam.tabix_index(args.out + ".nfrpos.bed.gz", preset="bed", force=True)
    if params.ins_track is None:
        pysam.tabix_compress(
            args.out + ".ins.bedgraph", args.out + ".ins.bedgraph.gz", force=True
        )
