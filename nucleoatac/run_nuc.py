...
import itertools
import pysam
from pyatac.utils import (
    shell_command,
    read_chrom_sizes_from_bam,
    read_chrom_sizes_from_fasta,
)
...
_writeFuncs = {
    "nucpos": _writeNucPos,
    "nucpos.redundant": _writeNucPosRedundant,
    "nucleoatac_signal": _writeNucSig,
    "nucleoatac_raw": _writeRaw,
    "nucleoatac_background": _writeBackground,
...
def run_nuc(args):
    ...
    for i in outputs:
        ...
        write_processes[i] = mp.Process(
            target=_writeFuncs[i], args=(write_queues[i], args.out)
        )
    ...
    for i in outputs:
        ...
        if i not in ["nucpos", "nucpos.redundant"]:
            ...
            pysam.tabix_index(
                args.out + "." + i + ".bedgraph.gz", preset="bed", force=True
            )
        else:
            pysam.tabix_compress(
                args.out + "." + i + ".bed", args.out + "." + i + ".bed.gz", force=True
            )
