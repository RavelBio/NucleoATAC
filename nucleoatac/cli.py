...
def nucleoatac_main(args):
    ...
    if call == "occ":
        ...
        print(
            "---------Obtaining nucleosome signal and calling positions----------------------"
        )
        run_nuc(args)
    elif call == "merge":
        ...
        print(
            "---------Calling NFR positions--------------------------------------------------"
        )
        run_nfr(args)
    elif call == "run":
        occ_args = parser.parse_args(
            list(
                map(
                    str,
                    [
        ...
        vprocess_args = parser.parse_args(
            ["vprocess", "--sizes", args.out + ".nuc_dist.txt", "--out", args.out]
        )
        nuc_args_list = [
            "nuc",
            "--bed",
        ...
        merge_args = parser.parse_args(
            [
                "merge",
                "--occpeaks",
                args.out + ".occpeaks.bed.gz",
                "--nucpos",
                args.out + ".nucpos.bed.gz",
                "--out",
                args.out,
            ]
        )
        nfr_args = parser.parse_args(
            [
                "nfr",
                "--bed",
                args.bed,
...
def nucleoatac_parser():
    ...
    argparser = argparse.ArgumentParser(
        description="%(prog)s -- Nucleosome Calling for ATAC-Seq",
...
def add_occ_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "occ", help="nucleoatac function:  Call nucleosome occupancy"
    )
    group1 = parser.add_argument_group("Required", "Necessary arguments")
    group1.add_argument(
        "--bed", metavar="bed_file", help="Peaks in bed format", required=True
    )
    group1.add_argument(
        "--bam", metavar="bam_file", help="Sorted (and indexed) BAM file", required=True
    )
    group1.add_argument(
        "--out", metavar="basename", help="give output basename", required=True
    )
    ...
    group4.add_argument("--fasta", metavar="genome_seq", help="Indexed fasta file")
    group4.add_argument(
        "--pwm",
        metavar="Tn5_PWM",
    ...
    group3.add_argument(
        ...
        metavar="int",
        default=251,
        help="upper limit in insert size. default is 251",
        type=int,
    )
    group3.add_argument(
        "--flank",
        metavar="int",
        default=60,
        help="Distance on each side of dyad to include for local occ calculation. Default is 60.",
        type=int,
    )
    group3.add_argument(
        "--min_occ",
...
def add_merge_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "merge", help="nucleoatac function: Merge occ and nuc calls"
    )
    group1 = parser.add_argument_group("Required", "Necessary arguments")
    group1.add_argument(
        "--occpeaks",
        metavar="occpeaks_file",
        required=True,
        help="Output from occ utility",
    )
    group1.add_argument(
        "--nucpos", metavar="nucpos_file", required=True, help="Output from nuc utility"
    )
    group2 = parser.add_argument_group("Options", "optional")
    group2.add_argument("--out", metavar="out_basename", help="output file basename")
    group2.add_argument(
        "--sep",
        metavar="min_separation",
        default=120,
...
def add_nfr_parser(subparsers):
    ...
    group6 = parser.add_argument_group(
        "Insertion track options", "Either input insertion track or bamfile"
    )
    group6.add_argument(
        "--ins_track",
        metavar="ins_file",
    ...
    group6.add_argument(
        "--bam", metavar="bam_file", help="Sorted (and indexed) BAM file"
    )
    group4 = parser.add_argument_group(
        "Bias calculation information",
        "Highly recommended. If fasta is not provided, will not calculate bias",
    ...
    group5.add_argument(
        ...
        metavar="float",
        default=0.1,
        help="Maximum mean occupancy for NFR. Default is 0.1",
        type=float,
    )
    group5.add_argument(
        "--max_occ_upper",
        metavar="float",
        default=0.25,
        help="Maximum for minimum of  upper bound occupancy in NFR. Default is 0.25",
        type=float,
    )
...
def add_vprocess_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "vprocess",
        help="nucleoatac function:  Make processed vplot to use for nucleosome calling",
    )
    group1 = parser.add_argument_group("Required", "Necessary arguments")
    ...
    group3.add_argument(
        ...
        metavar="int",
        default=105,
        help="lower limit (inclusive) in insert size. default is 105",
        type=int,
    )
    group3.add_argument(
        "--upper",
        metavar="int",
        default=251,
        help="upper limit (exclusive) in insert size. default 251",
        type=int,
    )
    group3.add_argument(
        "--flank",
        metavar="int",
        default=60,
        help="distance on each side of dyad to include",
        type=int,
    )
...
def add_nuc_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "nuc",
        help="nucleoatac function:  Call nucleosome positions and make signal tracks",
    )
    group1 = parser.add_argument_group("Required", "Necessary arguments")
    group1.add_argument(
        "--bed",
        metavar="bed_file",
        help="Regions for which \
        to do stuff.",
        required=True,
    )
    group1.add_argument(
        "--vmat", metavar="vdensity_file", help="VMat object", required=True
    )
    group1.add_argument(
    ...
    group2 = parser.add_argument_group(
        "Bias options", "If --fasta not provided, bias not calculated"
    )
    group2.add_argument("--fasta", metavar="genome_seq", help="Indexed fasta file")
    group2.add_argument(
        "--pwm",
        metavar="Tn5_PWM",
        help="PWM descriptor file. Default is Human.PWM.txt included in package",
    ...
    group3.add_argument(
        "--write_all", action="store_true", default=False, help="write all tracks"
    )
    group3.add_argument(
        "--not_atac",
        dest="atac",
    ...
    group5 = parser.add_argument_group(
        "Nucleosome calling parameters", "Change with caution"
    )
    group5.add_argument(
        "--min_z",
        metavar="float",
        default=3,
        help="Z-score threshold for nucleosome calls. Default is 3",
        type=float,
    )
    group5.add_argument(
        "--min_lr",
        metavar="float",
        default=0,
        help="Log likelihood ratio threshold for nucleosome calls. Default is 0",
        type=float,
    )
    group5.add_argument(
        "--nuc_sep",
    ...
    group5.add_argument(
        ...
        type=int,
        default=10,
        help="Standard deviation for smoothing. Affect the \
...
def add_run_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "run",
        help="Main nucleoatac utility-- runs through occupancy determination & calling nuc positions",
    )
    group1 = parser.add_argument_group("Required", "Necessary arguments")
    group1.add_argument(
        "--bed",
    ...
    group1.add_argument(
        "--out", metavar="output_basename", help="give output basename", required=True
    )
    group1.add_argument(
        "--fasta", metavar="genome_seq", help="Indexed fasta file", required=True
    )
