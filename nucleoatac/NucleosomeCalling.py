...
class SignalTrack(Track):
    ...
    def calculateSignal(self, mat, vmat):
        ...
        self.vals = signal.correlate(
            mat.get(vmat.lower, vmat.upper, mat.start + offset, mat.end - offset),
            vmat.mat,
            mode="valid",
        )[0]
...
class BiasTrack(Track):
    ...
    def calculateBackgroundSignal(self, mat, vmat, nuc_cov):
        ...
        self.cov.calculateCoverage(
            self.bias_mat, vmat.lower, vmat.upper, vmat.w * 2 + 1
        )
...
class SignalDistribution:
    ...
    def __init__(self, position, vmat, bias_mat, reads):
        ...
        self.reads = reads
        self.vmat = vmat
        bias_mat = bias_mat.get(
            vmat.lower, vmat.upper, position - vmat.w, position + vmat.w + 1
        )
    ...
    def simulateDist(self, numiters=1000):
        self.scores = [
            np.sum(self.simulateReads() * self.vmat.mat) for x in range(numiters)
        ]
...
class Nucleosome(Chunk):
    ...
    def getLR(self, nuctrack):
        mat = nuctrack.mat.get(
            nuctrack.params.lower,
            nuctrack.params.upper,
            self.start - nuctrack.params.vmat.w,
            self.start + nuctrack.params.vmat.w + 1,
        )
        null_mat = nuctrack.bias_mat.get(
            nuctrack.params.lower,
            nuctrack.params.upper,
            self.start - nuctrack.params.vmat.w,
            self.start + nuctrack.params.vmat.w + 1,
        )
        bias_mat = nuctrack.bias_mat_prenorm.get(
            nuctrack.params.lower,
            nuctrack.params.upper,
            self.start - nuctrack.params.vmat.w,
            self.start + nuctrack.params.vmat.w + 1,
    ...
    def getZScore(self, nuctrack):
        s = SignalDistribution(
            self.start, nuctrack.params.vmat, nuctrack.bias_mat, self.nuc_cov
        )
        std = s.analStd()
    ...
    def getFuzz(self, nuctrack):
        ...
        def fitNorm(guess, bound, sig):
            ...
            res = optimize.minimize(
                err_func, guess, args=a, bounds=bound, method="L-BFGS-B"
            )
            return res
        ...
        if len(means) == 1:
            ...
            bounds = (
                ...
                (2 ** 2, 50 ** 2),
                (0.001, max(sig) * 1.1),
                (means[1] - 10, means[1] + 10),
            )
            guesses = (
                nuctrack.params.smooth_sd ** 2,
                max(sig) * 0.9,
                means[0],
                nuctrack.params.smooth_sd ** 2,
                max(sig) * 0.9,
...
class NucChunk(Chunk):
    ...
    def makeBiasMat(self):
        ...
        self.bias_mat_prenorm = BiasMat2D(
            self.chrom,
            self.start - self.params.window,
            self.end + self.params.window,
            0,
    ...
    def getNucSignal(self):
        ...
        self.nuc_cov.calculateCoverage(
            self.mat, self.params.lower, self.params.upper, self.params.window
        )
        self.bias = BiasTrack(self.chrom, self.start, self.end)
        self.bias.calculateBackgroundSignal(
            self.bias_mat, self.params.vmat, self.nuc_cov
        )
    ...
    def getNFR(self):
        ...
        self.nfr_cov.calculateCoverage(
            self.mat, 0, self.params.lower, self.params.window
        )
    ...
    def fit(self):
        ...
        for nuc in self.sorted_nuc_keys:
            ...
            fit += norm(
                x,
                self.nuc_collection[nuc].fuzz ** 2,
                self.nuc_collection[nuc].weight,
                self.nuc_collection[nuc].fit_pos,
            )
        self.fitted = Track(
            self.chrom, self.start, self.end, "Fitted Nucleosome Signal"
        )
