...
class FragmentMixDistribution:
    ...
    def modelNFR(self, boundaries=(35, 115)):
        ...
        b = (
            np.where(
                self.fragmentsizes.get(self.lower, boundaries[1])
                == max(self.fragmentsizes.get(self.lower, boundaries[1]))
            )[0][0]
            + self.lower
        )
        ...
        self.nfr_fit0 = FragmentSizes(
            self.lower,
            self.upper,
            vals=gamma_fit(
                np.arange(self.lower, self.upper), whichres, res_param[whichres]
            ),
        )
        nfr = np.concatenate(
            (
    ...
    def plotFits(self, filename=None):
        ...
        fig = plt.figure()
        plt.plot(
            list(range(self.lower, self.upper)),
            self.fragmentsizes.get(),
            label="Observed",
        )
        plt.plot(
            list(range(self.lower, self.upper)), self.nfr_fit0.get(), label="NFR Fit"
        )
        plt.plot(
            list(range(self.lower, self.upper)),
            self.nuc_fit.get(),
            label="Nucleosome Model",
        )
        plt.plot(
            list(range(self.lower, self.upper)), self.nfr_fit.get(), label="NFR Model"
        )
        plt.legend()
...
class OccupancyTrack(Track):
    ...
    def calculateOccupancyMLE(self, mat, bias_mat, params):
        ...
        if offset < params.flank:
            raise Exception(
                "For calculateOccupancyMLE, mat does not have sufficient flanking regions"
            )(offset)
        ...
        for i in range(params.halfstep, len(self.vals), params.step):
            new_inserts = np.sum(
                mat.get(
                    lower=0,
                    upper=params.upper,
                    start=self.start + i - params.flank,
                    end=self.start + i + params.flank + 1,
                ),
                axis=1,
            )
            new_bias = np.sum(
            ...
            if sum(new_inserts) > 0:
                ...
                (
                    ...
                    self.lower_bound[left:right],
                    self.upper_bound[left:right],
                ) = calculateOccupancy(new_inserts, new_bias, params.occ_calc_params)

    def makeSmoothed(self, window_len=121, sd=20):
        self.smoothed_vals = smooth(
            self.vals, window_len, window="gaussian", sd=sd, mode="same", norm=True
        )
        self.smoothed_lower = smooth(
            self.lower_bound,
            window_len,
            window="gaussian",
            sd=sd,
...
class OccChunk(Chunk):
    ...
    def getFragmentMat(self):
        self.mat = FragmentMat2D(
            self.chrom,
            self.start - self.params.flank,
            self.end + self.params.flank,
            0,
            self.params.upper,
        )
    ...
    def callPeaks(self):
        ...
        peaks = call_peaks(
            self.occ.smoothed_vals, sep=self.params.sep, min_signal=self.params.min_occ
        )
        for peak in peaks:
    ...
    def getNucDist(self):
        ...
        for peak in list(self.peaks.keys()):
            sub = self.mat.get(
                start=self.peaks[peak].start - self.params.flank,
                end=self.peaks[peak].start + 1 + self.params.flank,
            )
