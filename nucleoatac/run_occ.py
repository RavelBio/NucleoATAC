...
def _occHelper(arg):
    ...
    try:
        ...
        out = (
            occ.getNucDist(),
            occ.occ,
            [occ.peaks[i] for i in sorted(occ.peaks.keys())],
        )
        occ.removeData()
...
def run_occ(args):
    ...
    if args.sizes is not None:
        ...
        fragment_dist.fragmentsizes = FragmentSizes(
            0, args.upper, vals=tmp.get(0, args.upper)
        )
    else:
    ...
    pysam.tabix_compress(
        args.out + ".occpeaks.bed", args.out + ".occpeaks.bed.gz", force=True
    )
    ...
    fig = plt.figure()
    plt.plot(
        list(range(0, args.upper)),
        dist_out.get(0, args.upper),
        label="Nucleosome Distribution",
    )
