...
class NFRChunk(Chunk):
    ...
    def getBias(self):
        """get bias"""
        self.bias = InsertionBiasTrack(self.chrom, self.start, self.end, log=True)
    ...
    def findNFRs(self):
        ...
        if self.chrom in tbx.contigs:
            for row in tbx.fetch(
                self.chrom, self.start, self.end, parser=pysam.asTuple()
            ):
        ...
        for j in range(1, len(nucs)):
            ...
            if (
                candidate.min_upper < self.params.max_occ_upper
                and candidate.occ < self.params.max_occ
            ):
