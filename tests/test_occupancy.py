...
from nucleoatac.Occupancy import (
    FragmentMixDistribution,
    calculateOccupancy,
    OccupancyCalcParams,
)
...
class Test_occupancy(TestCase):
    ...
    def setUp(self):
        ...
        self.fragment_dist.nfr_fit = FragmentSizes(
            0, 3, vals=np.array([0.5, 0.49, 0.01])
        )
        self.fragment_dist.nuc_fit = FragmentSizes(
            0, 3, vals=np.array([0.01, 0.49, 0.5])
        )
