...
class Test_FragmentMat2D(TestCase):
    ...
    def test_get(self):
        ...
        self.assertTrue(
            np.array_equal(
                x.get(start=505, end=507, lower=100, upper=102),
                np.array([[1, 0], [0, 0]]),
            )
        )
...
class Test_BiasMat(TestCase):
    ...
    def setUp(self):
        ...
        self.biastrack = InsertionBiasTrack(
            self.chunk.chrom, self.chunk.start, self.chunk.end
        )
        self.biastrack.read_track("example/example.Scores.bedgraph.gz")
        self.biasmat = BiasMat2D(
            self.chunk.chrom, self.chunk.start + 100, self.chunk.end - 100, 100, 200
        )
