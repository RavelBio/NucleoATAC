...
class Test_xcor(TestCase):
    ...
    def setUp(self):
        ...
        self.mat = FragmentMat2D(
            ...
            self.chunk.end + self.vmat.w,
            self.vmat.lower,
            self.vmat.upper,
        )
        self.mat.makeFragmentMat("example/example.bam")
        self.signal = Nuc.SignalTrack(
            self.chunk.chrom, self.chunk.start, self.chunk.end
        )
    ...
    def test_signal_calc1(self):
        ...
        a = np.sum(
            self.mat.get(
                start=self.chunk.start, end=self.chunk.start + self.vmat.w * 2 + 1
            )
            * self.vmat.mat
        )
    ...
    def test_signal_calc2(self):
        ...
        a = np.sum(
            self.mat.get(
                start=self.chunk.start + 100,
                end=self.chunk.start + 100 + self.vmat.w * 2 + 1,
            )
            * self.vmat.mat
        )
